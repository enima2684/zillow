...
#                      how='left')
   ...
   # remove propertycountylandusecode
    ...
    if not isTest :
        ...
        # rename over-estimations
        df.loc[df.bin==2,'bin'] = 1
...
def predict_on_test(mdl,scaler,cols_to_keep):
    ...
    for i, test in enumerate(tests):
        ...
        bin_result = pd.DataFrame(columns=['parcelid','p_under_est','p_over_est'])
        bin_result.parcelid = test.index.values
        bin_result['p_under_est'] = y[:,0]
        bin_result['p_over_est'] = y[:,1] 
    ...
    results.to_csv('data/s1_intermediate/output_s220.csv',index=False)
#    alert(1)
